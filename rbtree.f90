...
module rb_tree
    ...
    type rbtree_t
        type(rbtree_node_t), pointer :: root => null()
        integer                      :: count = 0
    contains
    ...
    abstract interface
        function format_func(key, value) result(s)
            class(*), intent(in) :: key, value
            character(len=:), allocatable :: s
        end function format_func
    end interface
    ...
    subroutine rbtree_create(t)
        ...
        logical :: tok
        ! root is null (empty tree) — a null root is defined as BLACK
    ...
    ! -----------------------------------------------------------------------
    ! Serialization: in-order traversal, entries separated by '|'
    ! The caller supplies a format_func that converts a key/value pair to a
    ! string.  The result can be compared directly with == or stored for later.
    ! -----------------------------------------------------------------------

    function rbtree_serialize(t, fmt) result(s)
        type(rbtree_t), intent(in) :: t
        procedure(format_func)     :: fmt
        character(len=:), allocatable :: s
        s = serialize_node(t%root, fmt)
    end function rbtree_serialize

    recursive function serialize_node(n, fmt) result(s)
        type(rbtree_node_t), pointer, intent(in) :: n
        procedure(format_func)                   :: fmt
        character(len=:), allocatable :: s
        ...
        ls = serialize_node(n%left,  fmt)
        rs = serialize_node(n%right, fmt)
        if (len(ls) > 0 .and. len(rs) > 0) then
            s = ls // '|' // fmt(n%key, n%value) // '|' // rs
        else if (len(ls) > 0) then
            s = ls // '|' // fmt(n%key, n%value)
        else if (len(rs) > 0) then
            s = fmt(n%key, n%value) // '|' // rs
        else
            s = fmt(n%key, n%value)
        end if
    end function serialize_node

    ! -----------------------------------------------------------------------
    ! Structural equality: parallel recursive traversal comparing keys and
    ! colours at every node.  Two trees built from the same insertion sequence
    ! are structurally equal; trees with the same key set but different
    ! insertion orders may differ in shape even though both are valid.
    ! For content-only comparison, serialize both and compare the strings.
    ! -----------------------------------------------------------------------

    logical function rbtree_equal(t1, t2, compare)
        type(rbtree_t), intent(in) :: t1, t2
        procedure(compare_func)    :: compare
        rbtree_equal = nodes_equal(t1%root, t2%root, compare)
    end function rbtree_equal

    recursive function nodes_equal(n1, n2, compare) result(r)
        logical :: r
        type(rbtree_node_t), pointer, intent(in) :: n1, n2
        procedure(compare_func)                  :: compare
        if (.not. associated(n1) .and. .not. associated(n2)) then
            r = .true.
            return
        end if
        if (.not. associated(n1) .or. .not. associated(n2)) then
            r = .false.
            return
        end if
        if (compare(n1%key, n2%key) /= 0 .or. n1%colour /= n2%colour) then
            r = .false.
            return
        end if
        r = nodes_equal(n1%left, n2%left, compare) .and. &
            nodes_equal(n1%right, n2%right, compare)
